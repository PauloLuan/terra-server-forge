...
public class FormEndpoint
{
   ...
   {
      TypedQuery<Form> findByIdQuery = em.createQuery("SELECT DISTINCT f FROM Form f WHERE f.id = :entityId ORDER BY f.id", Form.class);
   ...
   @GET
   @Produces("application/xml")
   public List<Form> listAll(@QueryParam("start") Integer startPosition, @QueryParam("max") Integer maxResult)
   {
      TypedQuery<Form> findAllQuery = em.createQuery("SELECT DISTINCT f FROM Form f ORDER BY f.id", Form.class);
